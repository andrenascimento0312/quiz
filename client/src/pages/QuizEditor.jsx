...
function QuizEditor() {
  ...
  // Carregar dados do quiz existente
  useEffect(() => {
    loadQuiz()
  }, [quizId])
  ...
  const loadQuiz = async () => {
    try {
      const response = await axios.get(`/quiz/admin/quizzes/${quizId}`)
      const quizData = response.data.quiz
      
      // Converter para formato do editor (usar tempId para controle do frontend)
      const formattedQuiz = {
        title: quizData.title,
        description: quizData.description,
        questions: quizData.questions.map(q => ({
          id: q.tempId, // Usar tempId para controle do frontend
          dbId: q.id, // Manter ID do banco para referência
          text: q.text,
          options: q.options,
      ...
      toast.error('Erro ao carregar quiz para edição')
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      await axios.put(`/quiz/admin/quizzes/${quizId}`, quiz)
      ...
      if (error.response) {
        ...
        toast.error('Erro inesperado ao atualizar quiz')
      }
    } finally {
      setLoading(false)
    }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-900">Carregando quiz...</h2>
        </div>
      </div>
    )
  }
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl p-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold mb-2">✏️ Editar Quiz</h1>
            <p className="text-blue-100">Atualize as informações e perguntas do seu quiz</p>
          </div>
        ...
        <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-8">
            ...
            <button
              ...
              className={`px-12 py-4 rounded-xl font-bold text-xl shadow-xl transform transition-all duration-200 min-w-48 ${
                loading || quiz.questions.length === 0
                  ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                  : 'bg-gradient-to-r from-blue-500 to-blue-600 text-white hover:from-blue-600 hover:to-blue-700 hover:scale-105 hover:shadow-2xl'
              }`}
            >
          ...
          {quiz.questions.length === 0 && (
            <p className="text-center text-red-500 text-sm mt-4">
              ⚠️ Adicione pelo menos uma pergunta para salvar o quiz
            </p>
          )}
        </div>
      </form>
